...
export const heroesSlice = createSlice({
	...
	reducers: {
		toggle: (state, action) => {
			const selectedHero = addOpponentsToHero(
				action.payload,
				state.allHeroes
			);
			const isSelected = state.selectedHeroesIds.includes(
				selectedHero.id
			);
			...
			let selectedHeroes;
			if (isSelected) {
				selectedHeroesIds = state.selectedHeroesIds.filter(
					(id) => id !== selectedHero.id
				);
				selectedHeroes = state.selectedHeroes.filter(
					(hero) => hero.id !== selectedHero.id
				);
			} else {
				selectedHeroesIds = [
					...state.selectedHeroesIds,
					selectedHero.id
				];
				selectedHeroes = [...state.selectedHeroes, selectedHero];
			}

			const counterHeroes = getStrongOpponentsFromSelectedHeroes(
				selectedHeroes,
				selectedHeroesIds
			);
